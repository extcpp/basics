...
template<typename T>
using tuple_wrap_t = typename tuple_wrap<T>::type;
...
template<typename T>
using tuple_unwrap_t = typename tuple_unwrap<T>::type;

template<typename... XS>
using tuple_cat_t = tuple_unwrap_t<decltype(std::tuple_cat(std::declval<tuple_wrap_t<XS>>()...))>;
// tuple wrapping - end
...
template<typename... XS>
constexpr std::size_t tuple_size_v = tuple_size<XS...>::value;
// tuple size - end

// replace in tuple
template<typename T, typename R, typename... XS>
using replace_in_tuple_t = std::tuple<std::conditional_t<std::is_same<XS, T>::value, R, XS>...>;
