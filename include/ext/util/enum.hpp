...
inline constexpr Enum underlying_to_enum(T value) noexcept {
    static_assert(std::is_enum_v<Enum>, "type is not an enum");
    static_assert(std::is_same_v<T, std::underlying_type_t<Enum>>,
                  "type to convert from and underlying type are not the same");
...
template <typename EnumOut, typename EnumIn, typename = std::enable_if_t<_detail_enum::are_enums_v<EnumIn, EnumOut>>>
