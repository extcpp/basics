 ...
 * @param[in]   in_string       LPCWSTR that will be converted to utf8 encoded
 * std::string
 * @param[in]   in_string_size  size of in_string if it is not null-terminated
 * @return                      size of in_string if it would be converted to
 * utf8 string
 */
inline int string_from_win_get_size(wchar_t const* in_string, int const in_string_size = -1) {
    return ::WideCharToMultiByte(CP_UTF8,              // UNIT    CodePage
 ...
 * @param[in|out]   out_string          char* buffer that will contain the
 * converted string
 *                                      it must have the size of out_string_size
 * @param[in]       out_string_size     int that describes the size of out
 * out_string
 * @param[in]       in_string           LPCWSTR that will be converted to utf8
 * encoded std::string
 * @param[in]       in_string_size      size of in_string if it is not
 * null-terminated
 * @return                              number of written bytes or 0 on failure
 */
inline int
    string_from_win(char* out_string, int out_string_size, wchar_t const* in_string, int const in_string_size = -1) {
    return ::WideCharToMultiByte(CP_UTF8,              // UNIT    CodePage
                                 WC_ERR_INVALID_CHARS, // DOWRD   dwFlags
                                 in_string,            // LPCWSTR lpWideCharStr
                                 in_string_size,       // int     cchWideChar
 ...
 * @param[in]   in_string       uft8 encoded const char* that will be converted
 * to LPWSTR
 * @param[in]   in_string_size  size of in_string if it is not null-terminated
 * @return                      size of in_string if it would be converted to
 * LPWSTR
...
inline int string_to_win_get_size(const char* in_string, int const in_string_size = -1) {
    return ::MultiByteToWideChar(CP_UTF8,        // UINT     CodePage
                                 0,              // DWORD    dwFlags
...
/*!
 * Functions that converts a given utf8 encoded char* string into a LPCWSTR
 * string
 * @param[in|out]   out_string          LPWSTR buffer that will contain the
 * converted string
 *                                      it must have the size of out_string_size
 * @param[in]       out_string_size     int that describes the size of out
 * out_string
 * @param[in]       in_string           utf8 encoded char* that will be
 * converted to LPWSTR
 * @param[in]       in_string_size      size of in_string if it is not
 * null-terminated
 * @return                              number of written bytes or 0 on failure
 */
inline int
    string_to_win(wchar_t* out_string, int out_string_size, const char* in_string, int const in_string_size = -1) {
    return ::MultiByteToWideChar(CP_UTF8,        // UINT     CodePage
                                 0,              // DWORD    dwFlags
                                 in_string,      // LPCSTR   lpMultiByteStr
                                 in_string_size, // int      cbMultiByte
...
/*!
 * Functions that converts a given utf8 encoded char* into an std::wstring (data
 * [LPWSTR])
 * @param[in]       in_string           utf8 encoded const char* string that
 * will be converted to std::wstring
 * @param[in]       in_string_size      size of in_string if it is not
 * null-terminated
