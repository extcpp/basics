...
struct defer {
    ...
    defer(Functor && c) noexcept(std::is_nothrow_move_constructible_v<Functor>)
        : functor(std::move(c)), active(true) { };
    defer(Functor const& c) noexcept(std::is_nothrow_copy_constructible_v<Functor>)
        : functor(c), active(true) { };
