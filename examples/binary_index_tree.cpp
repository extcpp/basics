...
int main(/*int argc, const char *argv[]*/) {
    ...
    for (std::size_t i = 0; i < 20; i++) {
        ...
        inc = ext::structures::detail::parent_update(inc);
        dec = ext::structures::detail::parent_interrogation(dec);
    }
    ...
    std::cout << "get:      ";
    for (std::size_t j = 0; j < arr.size(); ++j) {
        std::cout << std::setw(5) << ext::structures::bit_get<long>(begin, end, j);
    }
    std::cout << std::endl;
    ...
    std::cout << "cul:      ";
    for (std::size_t j = 0; j < arr.size(); ++j) {
        std::cout << std::setw(5) << ext::structures::bit_get_sum<long>(begin, end, j);
    }
    std::cout << std::endl;
